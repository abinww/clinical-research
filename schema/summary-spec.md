...
- extractor 永远只创建新文件，不得覆盖任何既有 raw 或 summary。同一来源重新提取也不得原地更新。
- 替换只能由 multi-extractor 执行：取得用户对该准确来源的明确授权后，先删除准确匹配的旧 raw/summary 对以及药品页、全部适应症页和根索引中的所有准确来源身份引用，再将该来源作为不存在的全新目标调用 extractor。未明确授权、无法唯一确定旧配对或无法清除全部准确索引引用时停止，不得覆盖、删除或部分替换。
...
文件名中的空格替换为 `_`，移除不适合路径的特殊字符。`drug_id` 必须与所属药品页一致，不得由摘要流程另行决定。
...
- 本地 PDF 来源保存解析后的绝对 POSIX 路径。在 Windows 上等价于 `Path.resolve().as_posix()`，例如 `C:/reports/trial.pdf`，不得只保存文件名、相对路径、反斜杠路径或另造路径标识。
- raw 与 summary 的配对身份由同一 `{drug_id}@{source_label}` 文件名确定；来源是否重复则比较上述 canonical `source` 值。近似来源只能提示人工判断，不能当作准确匹配自动替换。
...
| `indications` | 对象列表 | 必填，至少一个对象；每个对象包含 `indication_id`、`indication`，并可含 `line`、`biomarker` |
...
| `companies` | 数组 | 必填，使用根索引定义的 `company_id` |
...
- summary 初次生成时必须写 `verification: pending` 和 `verification_fail_count: null`，不得预先写 `passed`；示例中的 `passed/0` 表示独立审核完成后的最终状态。
...
- 正文必须按 `indications` 顺序分组，每个 `## {适应症}` 与一个对象一一对应。
...
| 适应症 | 数据项 | summary 中的值 | raw 证据 | 状态 | 问题 |
|--------|--------|----------------|----------|------|------|
| NSCLC 1L | ORR | 42.3% | "...ORR was 42.3%..." | PASS | — |
| NSCLC 2L | mPFS | 11.3 | 未找到 | FAIL | raw 中未出现该数值 |
| NSCLC 2L | >=3 TRAE | 25.0% | "...grade 3 or higher TEAEs..." | WARN | raw 为 TEAE，summary 写 TRAE |
```

- summary 中每个临床数值、试验事实和关键分组都必须有审核行，并标明所属适应症。
...
- 只有 `verification: passed`、`verification_fail_count: 0`、审核章节存在且审核章节中没有任何 `FAIL` 的摘要，才可更新药品页、适应症页或根索引。
...
- [ ] `indications` 是至少含一个对象的列表，正文按对象逐一分组。
